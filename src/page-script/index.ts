...
export function isInteractiveNode(node: Node | null): boolean {
  ...
  try {
    ...
    if (curr instanceof Element) {
      ...
      // Fast-path: O(1) check for standard HTML form & canvas elements before running safeClosest CSS selector tree traversal
      if (
        tag === 'INPUT' ||
        tag === 'TEXTAREA' ||
        tag === 'SELECT' ||
    ...
    if (composedPathFn) {
      ...
      if (Array.isArray(path) && path.length > 0) {
        return path.some((item) => {
          ...
          // Fast-path: O(1) tag check for standard interactive elements to bypass safeMatches CSS selector evaluation
          if (
            tag === 'INPUT' ||
            tag === 'TEXTAREA' ||
            tag === 'SELECT' ||
