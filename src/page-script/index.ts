...
export function isInteractiveNode(node: Node | null): boolean {
  ...
  try {
    ...
    if (curr instanceof Element) {
      if (FAST_INTERACTIVE_TAGS.has(curr.tagName)) return true;
...
// Fast-path interactive event check. Fast-tag check skips selector evaluation for standard controls.
    ...
    if (composedPathFn) {
      ...
      if (Array.isArray(path) && path.length > 0) {
        return path.some((item) => {
          ...
          if (FAST_INTERACTIVE_TAGS.has(item.tagName)) return true;
...
// biome-ignore lint/style/useConst: reassigned dynamically during channel handshake
